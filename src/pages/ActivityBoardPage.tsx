  ...
  return (
    <PageShell description={t('board.description')} eyebrow="Activity Board" title={t('board.title')}>
        ...
        <div className="rounded-[1.25rem] bg-theme-card/82 p-4 backdrop-blur">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-2">
              <Badge className="bg-theme-main text-white"><UsersRound size={13} />{t('board.badge')}</Badge>
              <p className="text-sm leading-6 text-theme-muted">{t('board.intro')}</p>
            </div>
      ...
      <div className="grid gap-2 text-xs font-black sm:grid-cols-2">
        <Link className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-theme-card/90 px-3 py-2 text-theme-main-dark shadow-sm transition hover:bg-theme-accent-soft" to="/my-board"><UsersRound size={15} />{t('board.myPosts')}</Link>
        <Link className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-theme-card/90 px-3 py-2 text-theme-main-dark shadow-sm transition hover:bg-theme-accent-soft" to="/my-interests"><Eye size={15} />{t('board.myInterests')}</Link>
      </div>
        ...
        {visiblePosts.map((post) => (
          <Card className="space-y-3" key={post.id}>
            ...
            <div className="flex flex-wrap gap-2 text-xs font-bold text-theme-muted">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-theme-accent-soft/60 px-2.5 py-1"><UsersRound size={14} />{t('board.interested')} {post.interest_count}件</span>
            ...
            {post.room_id ? <p className="rounded-xl bg-theme-accent-soft/60 p-3 text-xs font-black text-theme-main-dark">{t('board.fromRoom')}</p> : null}
            {isOwnPost(post) ? (
            ...
            <div className="flex items-center justify-between gap-3 border-t border-white/60 pt-3">
              ...
              <Link className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-theme-accent-soft px-4 py-2 text-sm font-black text-theme-main-dark" to={`/board/${post.id}`}>{t('board.viewDetails')}</Link>
            </div>
          </Card>
        ))}
      </div>
