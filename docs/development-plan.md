...
## β版テスター向け最低限ページ Phase 1

- 少人数テスター投入前の最低限ページとして、`/test-guide`、`/terms`、`/privacy` を追加する。
- `/test-guide` は、ログイン、プロフィール、ルーム、募集ボード、参加希望、承認、DM、通知、安心機能を順番に確認できるチェックリスト風ガイドとする。
- `/terms` と `/privacy` は、β版テスト向けの暫定ページとして提供し、正式な法務文書ではなく、正式公開前に内容を見直す前提とする。
- `/settings` の「安心・運営」からテスターガイド、利用規約、プライバシーポリシーへ移動できる導線を追加する。
- 新規SQLやDB schema変更、同意チェック保存、利用規約同意テーブルはこのPhaseでは追加しない。

## テスター招待・フィードバック回収 Phase 1

- βテスター向けに `/test-guide` のフィードバック案内を強化し、`/home` と `/settings` からテスターガイドへアクセスしやすくした。

## 招待コード運用の確認・整備 Phase 1

- βテスター招待前に、`/login`、`/settings`、`/admin`、`/test-guide` の招待コードまわりの文言と導線を軽く整備した。
- 既存DB構造や招待コード発行ロジックは変更せず、招待制β版としての案内、テスター向け説明、管理者が共有しやすい短い案内文を強化した。

## 日本語 / English 切り替え Phase 1

- 日本語 / English 切り替え Phase 1 として、`/settings` の言語設定、localStorage保存、初回ページ・ログイン・テスターガイド等の主要入口文言を切り替えられるようにした。
- 未対応ページは日本語を維持し、翻訳キーがない場合も日本語にフォールバックする軽量な自作i18n helperで運用する。

## 日本語 / English 切り替え Phase 2

- Home / Discover / Profile detail / Rooms の主要導線文言を、日本語 / English 切り替え対象にした。
- 未対応ページは既存の日本語表示と i18n の日本語フォールバックを維持し、全ページ完全翻訳は今後の拡張として扱う。

## 日本語 / English 切り替え Phase 3

- 日本語 / English 切り替え Phase 3 として、Board / My posts / My interests / Notifications / Messages の主要文言を切り替え対象にした。
- フォーム全文や管理画面全文は未対応で、日本語フォールバックを維持している。

## 日本語 / English 切り替え Phase 4

- 日本語 / English 切り替え Phase 4 として、Settings 画面のセクション見出し、カードタイトル、説明文、通知なしバッジ、ログアウト周辺を切り替え対象にした。
