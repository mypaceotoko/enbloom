-- Harden the activity/direct DM RPCs and keep their exposed argument names stable.
-- This migration is intentionally repeatable so production projects that were created
-- before generated match-pair columns existed can still run the DM RPCs reliably.

alter table public.matches
  add column if not exists user_low_id uuid generated always as (least(user1_id, user2_id)) stored;

alter table public.matches
  add column if not exists user_high_id uuid generated always as (greatest(user1_id, user2_id)) stored;
